...
export default function FinalSummary2({ choices, quotes, category }) {
  const [meterW, setMeterW] = useState(0)
  const [quoteIdx, setQuoteIdx] = useState(0)
  const [revealedRows, setRevealedRows] = useState(0)

  const loveQuotes = quotes || [
  ...
  return (
    <div className="page page-enter">
      ...
      <FlowerBloom active={true} />
      <StickerAnimation
        active={true}
        category={STICKER_MAP[category] || 'love'}
        count={10}
      />
      ...
      <div className="glass final-card">
        ...
        <h2 className="font-script" style={{ fontSize: '2.3rem', color: 'var(--t-primary, var(--pink))', marginBottom: 6 }}>
          Бүх зүйл бэлэн! 🎉
        </h2>
                  ...
                  {Array.isArray(choices[f.key]) ? (
                    <div className="sr-multi-vals">
                      {choices[f.key].map((v, vi) => (
                        <span key={vi} className="sr-tag">{v}</span>
                      ))}
                    </div>
                  ) : (
        ...
        {/* Signature */}
        <div className="font-vibes" style={{ fontSize: '1.6rem', color: 'var(--t-soft, var(--rose))', marginBottom: 8 }}>
          Forever Together
        </div>
